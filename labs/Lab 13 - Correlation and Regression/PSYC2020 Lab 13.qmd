...
CIOS: 1% increase to total lab grade if we get to 85% completion for the lab section!

Lab 14: I will include a few bonus questions on the lab 14 assignment
...
When you modify a variable, don't forget to assign the new dataset to an object so the modification is preserved.
...
iris_small <- iris_small |>
  mutate(diff_x = Sepal.Length - x_bar,
         diff_y = Sepal.Width - y_bar,
...
If we want to see the correlations between many variables at once, we can get a correlation matrix

Use `select()` to get just the numeric columns you want, then pass it into `cor()`
...
[What are the strongest relationships here?]{.fragment}
